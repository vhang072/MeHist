path1 <- "Y:/4.basic_data/TCGA_PancanAtlas/methylation_L1/LAML/"
path2 <- "Y:/4.basic_data/TCGA_PancanAtlas/methylation_L1/Primary/"
cancerTypes <- c("LAML",
                 "ACC",
                 "BLCA",
                 "BRCA",
                 ...
                 "THYM",
                 "UCEC")
tumorFiles <- c(paste0(path1,"_L1Probe_TCGA-",cancerTypes[1],".txt"))
for (x in 2:length(cancerTypes)){
  tumorFiles <- c(tumorFiles,paste0(path2,"_L1Probe_TCGA-",cancerTypes[x],".txt"))
}
lostTypes = c("MESO","TGCT","UCS","UVM")#without matched normal 
uncertainTypes = c("DLBC","GBM","KICH","SKCM","STAD") # used outside normal data
path3 = "Y:/4.basic_data/TCGA_PancanAtlas/methylation_L1/SolidNormal/"
path4 = "Y:/4.basic_data/TCGA_PancanAtlas/methylation_L1/GEONormal/"
...
for (x in 1:length(solidTypes)){
  if (nchar(solidTypes[x])>10){
    normalFiles <- c(normalFiles,paste0(path4,"_L1Probe_",solidTypes[x],".txt"))
  } else{
    normalFiles <- c(normalFiles,paste0(path3,"_L1Probe_",solidTypes[x],".txt"))
  }
}
...
for (x in used_lines){
  ...
  write.table(merged,
              paste0("Y:/4.basic_data/TCGA_PancanAtlas/methylation_L1/Diff_TCGA/_DiffL1probe_",cancerTypes[x],".txt"),
