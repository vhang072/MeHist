...
cor_mean <- NULL
path <- "methylation_soloWCGW"#methylation_opensea
type1 <- "soloWCGWprobe" #soloWCGWprobe;OpenSeaprobe
type <- "PMDsoloWCGWProbe"#PMDsoloWCGWProbe;OpenseaProbe
range_start <- 4 #2;4
for (i in used_lines){
  info <- read.table(paste0("Y:/4.basic_data/TCGA_PancanAtlas/",path,"/Diff_CCLE/_Diff",type1,"_",cancerTypes[i],".txt"),
                     sep = "\t",header = T)
  data <- read.table(paste0("Y:/4.basic_data/TCGA_PancanAtlas/",path,"/CellLines/_",type,"_450_allprobe_split_",cancerTypes[i],".txt"),
                     sep = "\t",header = T)
  data_colrange <- range_start:ncol(data)
  ...
  write.table(mean_vals,paste0("Y:/4.basic_data/TCGA_PancanAtlas/",path,"/Diff_CCLE/_Diff",type,"Mean_",cancerTypes[i],".txt"),
...
write.table(cor_mean,paste0("Y:/4.basic_data/TCGA_PancanAtlas/",path,"/Diff_CCLE/__Diff",type,"CorSummary.txt"),
            sep = "\t",row.names = F,col.names = T, quote = F)
