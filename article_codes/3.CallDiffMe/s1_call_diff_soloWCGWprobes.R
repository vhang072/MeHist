path1 <- "Y:/4.basic_data/TCGA_PancanAtlas/methylation_soloWCGW/LAML/"
path2 <- "Y:/4.basic_data/TCGA_PancanAtlas/methylation_soloWCGW/Primary/"
cancerTypes <- c("LAML",
                 "ACC",
                 "BLCA",
                 "BRCA",
                 ...
                 "THYM",
                 "UCEC")
tumorFiles <- c(paste0(path1,"_PMDsoloWCGWProbe_TCGA-",cancerTypes[1],".txt"))
for (x in 2:length(cancerTypes)){
  tumorFiles <- c(tumorFiles,paste0(path2,"_PMDsoloWCGWProbe_TCGA-",cancerTypes[x],".txt"))
}
lostTypes = c("MESO","TGCT","UCS","UVM")#without matched normal 
uncertainTypes = c("DLBC","GBM","KICH","SKCM","STAD") # used outside normal data
path3 = "Y:/4.basic_data/TCGA_PancanAtlas/methylation_soloWCGW/SolidNormal/"
path4 = "Y:/4.basic_data/TCGA_PancanAtlas/methylation_soloWCGW/GEONormal/"
...
for (x in 1:length(solidTypes)){
  if (nchar(solidTypes[x])>10){
    normalFiles <- c(normalFiles,paste0(path4,"_PMDsoloWCGWProbe_",solidTypes[x],".txt"))
  } else{
    normalFiles <- c(normalFiles,paste0(path3,"_PMDsoloWCGWProbe_",solidTypes[x],".txt"))
  }
}
...
for (x in used_lines){
  ...
  for (idx in 1:nrow(tumor_select)){
    tumorme <- tumor_select[idx,4:ncol(tumor_select)]
    normalme <- normal_select[idx,4:ncol(normal_select)]
  ...
  merged <- cbind(tumor_select[,1:3],diffprobeinfo)
  write.table(merged,
              paste0("Y:/4.basic_data/TCGA_PancanAtlas/methylation_soloWCGW/Diff_TCGA/_DiffsoloWCGWprobe_",cancerTypes[x],".txt"),
