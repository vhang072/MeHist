...
for (i in used_lines){
  print(cancerTypes[i])
  info0 <- read.table(paste0("Y:/4.basic_data/TCGA_PancanAtlas/methylation_cgiprobe/Diff_CCLE/_DiffCGIprobe_",cancerTypes[i],".txt"),
                     sep = "\t",header = T)
  info <- na.omit(info0)
  ...
  hyper_wide <- info[(info$Mean_T > info$Mean_N) & (info$AberrantFrac_T_0.2 > 0.1) & (info$Mean_N < 0.3),]
  hypo_wide <- info[(info$Mean_T < info$Mean_N) & (info$AberrantFrac_T_0.2 > 0.1) & (info$Mean_N > 0.7),]
  ...
  write.table(corrs,paste0("Y:/4.basic_data/TCGA_PancanAtlas/methylation_cgiprobe/Diff_CCLE/_CGIprobeHyperCors_",cancerTypes[i],".txt"),
  ...
  write.table(corrs2,paste0("Y:/4.basic_data/TCGA_PancanAtlas/methylation_cgiprobe/Diff_CCLE/_CGIprobeHypoCors_",cancerTypes[i],".txt"),
              sep = "\t",row.names = T,col.names = T, quote = F)
