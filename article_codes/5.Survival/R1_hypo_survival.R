...
median_num <- NULL
pancancer_out <- NULL
pdf("I:/Projects/P7.Methy-PanCancer/pics/hyposurvival.pdf",height = 6,width = 6)
for (idx in 1:length(cancerTypes)){
  ...
  median_num <- c(median_num,median(as.numeric(unname(data[4,]))))
  colnm <- NULL
  colx <- NULL
  ...
  out <- EMGauss(cdr_data$hypo_num,K = 2,Means = c(0,100),SDs = c(20,20),MaxNumberofIterations =20)
  
  thred <- quantile(cdr_data$hypo_num,out$Weights[1])
  
  cdr_data$hypo_label <- factor(ifelse(cdr_data$hypo_num > thred, "Highly_Hypo","Control"),
                                 levels = c("Control","Highly_Hypo"))
  if ((sum(cdr_data$hypo_label ==  "Highly_Hypo")>=5) & (sum(cdr_data$hypo_label ==  "Control")>=5)) {
  } else{
    cdr_data$hypo_label <- factor(ifelse(cdr_data$hypo_num > median(cdr_data$hypo_num),"Highly_Hypo","Control"),
                                  levels = c("Control","Highly_Hypo"))
  }
  
  write.table(cdr_data,paste0("Y:/4.basic_data/TCGA_PancanAtlas/methylation_cgiprobe/Survival_hypo/HyperDichotomy_",cancerTypes[idx],".txt"),
              sep = "\t",quote = F)
  ...
  if ((sum(cdr_data_dss$hypo_label ==  "Highly_Hypo")>=5) & (sum(cdr_data_dss$hypo_label ==  "Control")>=5)) {
    coxfit <- coxph(Surv(DSS.time,DSS) ~ hypo_label, data=cdr_data_dss)
    x.out <- summary(coxfit)
    tmp_out <- c(tmp_out,x.out$conf.int[1],x.out$sctest[3])
    
    fit <- survfit(Surv(DSS.time,DSS) ~ hypo_label, data=cdr_data_dss)
    p <- ggsurvplot(fit,
                    pval = TRUE, conf.int = F,
                    risk.table = TRUE, # Add risk table
                    risk.table.col = "strata", # Change risk table color by groups
  ...
  if ((sum(cdr_data_dfi$hypo_label ==  "Highly_Hypo")>=5) & (sum(cdr_data_dfi$hypo_label ==  "Control")>=5)) {
    coxfit <- coxph(Surv(DFI.time,DFI) ~ hypo_label, data=cdr_data_dfi)
    x.out <- summary(coxfit)
    tmp_out <- c(tmp_out,x.out$conf.int[1],x.out$sctest[3])
    
    fit <- survfit(Surv(DFI.time,DFI) ~ hypo_label, data=cdr_data_dfi)
    p <- ggsurvplot(fit,
                    pval = TRUE, conf.int = F,
                    risk.table = TRUE, # Add risk table
                    risk.table.col = "strata", # Change risk table color by groups
  ...
  if ((sum(cdr_data_pfi$hypo_label ==  "Highly_Hypo")>=5) & (sum(cdr_data_pfi$hypo_label ==  "Control")>=5)) {
    coxfit <- coxph(Surv(PFI.time,PFI) ~ hypo_label, data=cdr_data_pfi)
    x.out <- summary(coxfit)
    tmp_out <- c(tmp_out,x.out$conf.int[1],x.out$sctest[3])
    
    fit <- survfit(Surv(PFI.time,PFI) ~ hypo_label, data=cdr_data_pfi)
    p <- ggsurvplot(fit,
                    pval = TRUE, conf.int = F,
                    risk.table = TRUE, # Add risk table
                    risk.table.col = "strata", # Change risk table color by groups
...
write.table(pancancer_out, "Y:/4.basic_data/TCGA_PancanAtlas/methylation_cgiprobe/Survival_hypo/Hypo_survivalHRPvalue_table.txt",
            sep = "\t",quote = F)
## distributions
...
library(ggplot2)
pdf("I:/Projects/P7.Methy-PanCancer/pics/hyper_hypo_num_byhypo.pdf",height = 4,width = 16)
