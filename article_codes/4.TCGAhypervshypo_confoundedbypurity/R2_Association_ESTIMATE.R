...
for (x in 1:length(cancerTypes)){
  ...
  if (length(overlap_sp)>=20){
    ...
    write.table(out, paste0("Y:/4.basic_data/TCGA_PancanAtlas/methylation_cgiprobe/Diff_TCGA/",fold,"_MeanCGIprobe/_SigPur.",
                            cancerTypes[x],".CGIMean_",fold,".txt"),sep = "\t",row.names = T, col.names = T)
...
library(paletteer)
pdf("I:/Projects/P7.Methy-PanCancer/pics/tcga_purity_pertumor/meanval_corr_items_estimate.pdf",height = 4,width = 7)
...
library(paletteer)
pdf("I:/Projects/P7.Methy-PanCancer/pics/tcga_purity_pertumor/num_corr_items_estimate.pdf",height = 4,width = 7)
