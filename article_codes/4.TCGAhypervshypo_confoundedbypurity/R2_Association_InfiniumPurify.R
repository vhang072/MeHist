...
#methylation enzymes
genelist3 <- c("ENSG00000130816|DNMT1","ENSG00000119772|DNMT3A","ENSG00000088305|DNMT3B","ENSG00000142182|DNMT3L",
...
for (x in 1:length(cancerTypes)){
  ...
  for (idx in 1:ncol(exp)){
    sp_name <- substr(colnames(exp)[idx],1,16)
  ...
  for (idx in 1:ncol(data)){
    sp_name <- substr(colnames(data)[idx],1,16)
    if (sp_name %in% data_colnames == F){
      data_colnames <- c(data_colnames, sp_name)
      used_cols <- c(used_cols,idx)
    }
  }
  data <- data[,used_cols]
...
cols.t <- c("Hypo","Hypoxia","Proliferation","CellCycle","DNMT1","DNMT3A","DNMT3B","DNMT3L","TET1","TET2","TET3")
...
tmp.order <- order(as.numeric(cor_mat[,12]),decreasing = T)
...
library(paletteer)
pdf("I:/Projects/P7.Methy-PanCancer/pics/tcga_purity_pertumor/meanval_corr_items_infiniumPurity.pdf",height = 7,width = 7)
...
tmp.order <- order(as.numeric(cor_mat2[,12]),decreasing = T)
...
library(paletteer)
pdf("I:/Projects/P7.Methy-PanCancer/pics/tcga_purity_pertumor/num_corr_items_infiniumPurity.pdf",height = 7,width = 7)
