...
cor_hypo <- NULL
pval_hypo <- NULL
purity <- read.table("Y:/4.basic_data/TCGA_PancanAtlas/tumor_purity/ESTIMATE/ESTIMATE.txt",
                     sep = "\t",header = T)
purity_rmna <- purity[is.na(purity$tumor.purity) == F,]



rownames(purity_rmna) <- purity_rmna$ID
...
for (x in 1:length(cancerTypes)) {
  ...
  if (length(overlap_sps)>=10){
    purity_val <- purity_rmna[overlap_sps,"tumor.purity"]
    data_val <- data1_curated[,overlap_sps]
    out <- rbind(data_val,purity_val)
    out <- as.data.frame(out)
    write.table(out,paste0("Y:/4.basic_data/TCGA_PancanAtlas/methylation_cgiprobe/Diff_TCGA/ESTIMATE_MeanCGIprobe/",
                           cancerTypes[x],".CGIMean_ESTIMATE.txt"),sep = "\t",row.names = F, col.names = T)
  }
