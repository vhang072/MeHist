...
cor_hypo <- NULL
pval_hypo <- NULL
purity <- read.table("Y:/4.basic_data/TCGA_PancanAtlas/tumor_purity/LUMP_associated/tumor_purity_refer1.txt",
                     sep = "\t",header = T)
purity_rmna <- purity[is.na(purity$LUMP) == F,]
purity_rownames <- NULL
library(stringr)
for (idx in 1:nrow(purity_rmna)){
  purity_rownames <- c(purity_rownames,str_replace_all(purity_rmna$Sample.ID[idx],"-","."))
}
...
for (x in 1:length(cancerTypes)) {
  ...
  if (length(overlap_sps)>=10){
    purity_val <- purity_rmna[overlap_sps,"LUMP"]
    data_val <- data1_curated[,overlap_sps]
    out <- rbind(data_val,purity_val)
    out <- as.data.frame(out)
    write.table(out,paste0("Y:/4.basic_data/TCGA_PancanAtlas/methylation_cgiprobe/Diff_TCGA/LUMP_MeanCGIprobe/",
                           cancerTypes[x],".CGIMean_LUMP.txt"),sep = "\t",row.names = F, col.names = T)
  }
