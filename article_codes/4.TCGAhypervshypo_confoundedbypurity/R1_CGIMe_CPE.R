...
purity <- read.table("Y:/4.basic_data/TCGA_PancanAtlas/tumor_purity/LUMP_associated/tumor_purity_refer1.txt",
                     sep = "\t",header = T)
purity_rmna <- purity[is.na(purity$CPE) == F,]
...
for (x in 1:length(cancerTypes)) {
  ...
  if (length(overlap_sps)>=10){
    purity_val <- purity_rmna[overlap_sps,"CPE"]
    data_val <- data1_curated[,overlap_sps]
    out <- rbind(data_val,purity_val)
    out <- as.data.frame(out)
    write.table(out,paste0("Y:/4.basic_data/TCGA_PancanAtlas/methylation_cgiprobe/Diff_TCGA/CPE_MeanCGIprobe/",
                           cancerTypes[x],".CGIMean_CPE.txt"),sep = "\t",row.names = F, col.names = T)
  }
