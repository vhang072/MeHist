...
cor_hypo <- NULL
pval_hypo <- NULL
purity <- read.table("Y:/4.basic_data/TCGA_PancanAtlas/tumor_purity/ABSOLUTE/TCGA_mastercalls.abs_tables_JSedit.fixed.txt",
                     sep = "\t",header = T)
purity_rmna <- purity[is.na(purity$purity) == F,]


purity_rownames <- NULL
library(stringr)
for (idx in 1:nrow(purity_rmna)){
  tmp.name <- str_replace_all(purity_rmna$array[idx],"-",".")
...
for (x in 1:length(cancerTypes)) {
  ...
  for (idx in 2:ncol(data1)){
    sp_now <- substr(colnames(data1)[idx],1,15)
  ...
  if (length(overlap_sps)>=10){
    purity_val <- purity_rmna[overlap_sps,"purity"]
    data_val <- data1_curated[,overlap_sps]
    out <- rbind(data_val,purity_val)
    out <- as.data.frame(out)
    write.table(out,paste0("Y:/4.basic_data/TCGA_PancanAtlas/methylation_cgiprobe/Diff_TCGA/ABSOLUTE_MeanCGIprobe/",
                           cancerTypes[x],".CGIMean_ABSOLUTE.txt"),sep = "\t",row.names = F, col.names = T)
  }
