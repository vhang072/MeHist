...
for op,val in opts:
    if op == '-i':
        ...
        print('Usage: python building_cpg_index.py -i hg38.fa -o ./cpg_pos/\n\
...
def detercpgpos(chrom_seq,final_name):
    ...
    for i in range(0,chrom_len-1):
        ...
        if chrom_seq[i:(i+2)] == 'CG':
            tmp_pos = [[final_name,i+1, cpg_rank]]
            cpg_rank += 1
            cpgpos += tmp_pos
        else:
            pass
...
if __name__ == "__main__":
    ...
    out_pos = pd.DataFrame(cpg_pos,columns=['chrom','pos_C','cpg_rank'])
    out_pos.to_csv(outfile+'/'+'cpgpos_wholegenome'+'.pos',sep = '\t',header = False,index = False)
